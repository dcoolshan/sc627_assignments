...
def computeDistancePointToSegment(x1,y1,x2,y2,x,y):
	...
	if(cond1>0 and cond2>0):
		...
		w=2
		# w=2 implies it is at minimum distance from point (x2,y2) of line segment between pt 1 and 2
	elif(cond1<0 and cond2<0):
		dist=sqrt((x-x1)**2+(y-y1)**2)
		w=1
		# w=1 implies it is at minimum distance from point (x1,y1) of line segment between pt 1 and 2
	else:
		dist=computeDistancePointToLine(x1,y1,x2,y2,x,y)
		w=0
		# w=0 implies it is at minimum distance from line segment between pt 1 and 2, in direction normal to segment
	return dist,w
...
def computeDistancePointToPolygon(poly,x,y):
	...
	for i in range(len(poly)):
		...
		if(i==len(poly)-1):
			...
			lis_dist.append(d)
			lis_w.append(w)
			# we are creating two lists, one storing distance and second specifies "from where is that minimum distance measured"
	...
	# ind=i means the minimum distance of point from polygon is from line segment between point i and i+1
	# if ind=len(poly)-1 then the minimum distance of point from polygon is from line segment between point i and 0
